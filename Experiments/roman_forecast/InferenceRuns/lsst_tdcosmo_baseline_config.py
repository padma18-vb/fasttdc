...
# file locations
static_dv_file = 'InferenceRuns/lsst_tdcosmo_baseline/static_datavectors_seed'+str(RANDOM_SEED)+'.json'
...
BACKEND_PATH = 'InferenceRuns/lsst_tdcosmo_baseline/LCDM_seed'+str(RANDOM_SEED)+'_backend.h5'
RESET_BACKEND=True
...
lsst_aper_quads_avail = np.where(
    (lsst_df['point_source_parameters_num_images'].to_numpy() == 4) &
    ((np.abs(lsst_df['td01'].to_numpy()) > 5.) |  # 5-day LSST time-delay threshold
     (np.abs(lsst_df['td02'].to_numpy()) > 5.) | 
     (np.abs(lsst_df['td03'].to_numpy()) > 5.)) &
...
likelihood_configs = {
    ...
    'lsst_aper_quads':{
        ...
        'cosmo_model':COSMO_MODEL,
        'td_meas_error_percent':None,
        'td_meas_error_days':5., # 5-Day LSST Meas. Precision
        'kappa_ext_meas_error_value':0.05,
